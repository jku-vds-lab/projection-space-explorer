...
RUN python3 -m pip install numba==0.53
RUN python3 -m pip install pillow
RUN python3 -m pip install matplotlib
RUN python3 -m pip install pandas
RUN python3 -m pip install umap-learn
RUN python3 -m pip install opentsne
RUN apt-get update && apt-get install -y --no-install-recommends wget git
RUN python3 -m pip install git+https://github.com/dn1z/pgn2gif.git
...
WORKDIR /pse/notebooks/chess
EXPOSE 8888

RUN python3 -m ipykernel.kernelspec

CMD ["bash", "-c", "source /etc/bash.bashrc && jupyter notebook --notebook-dir=/pse/notebooks/chess --ip 0.0.0.0 --no-browser --allow-root"]