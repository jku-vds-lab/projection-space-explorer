...
const LayeringSystem_1 = require("./LayeringSystem");
const StoriesDuck_1 = require("../Ducks/StoriesDuck");
const Mapping_2 = require("../Utility/Colors/Mapping");
// @ts-ignore
const atlas_png_1 = require("../../../textures/sprites/atlas.png");
...
exports.imageFromShape = imageFromShape;
...
class LineVisualization {
    constructor(segments, lineColorScheme) {
        ...
        this.grayedLayerSystem = new LayeringSystem_1.LayeringSystem(this.segments.length);
        // selection layer
    ...
    storyTelling(stories) {
        ...
        if (stories && stories.trace) {
            ...
            stories.trace.mainPath.forEach(cluster => {
                StoriesDuck_1.StoriesUtil.retrieveCluster(stories, cluster).indices.forEach(i => {
                    lineIndices.add(stories.vectors[i].__meta__.lineIndex);
                });
            });
            lineIndices.forEach(lineIndex => {
    ...
    update() {
        this.segments.forEach((segment, si) => {
            ...
            segment.__meta__.lineMesh.visible = segment.__meta__.detailVisible
                ...
                && UtilityFunctions_1.valueInRange(segment.vectors.length, this.pathLengthRange);
        });
    }
}
exports.LineVisualization = LineVisualization;
class PointVisualization {
    constructor(vectorColorScheme, dataset, size, lineLayerSystem, segments) {
        ...
        this.grayedLayerSystem = new LayeringSystem_1.LayeringSystem(dataset.vectors.length);
        // selection layer
        this.grayedLayerSystem.registerLayer(5, true);
        this.grayedLayerSystem.setLayerActive(5, false);
        // trace layer
    ...
    createMesh(data, segments) {
        ...
        var pointMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                scale: { value: 1.0 },
                atlas: {
                    value: new THREE.TextureLoader().load(atlas_png_1.default)
                }
            },
            transparent: true,
    ...
    groupHighlight(samples) {
        if (samples && samples.length > 0) {
            this.grayedLayerSystem.clearLayer(5, true);
            this.grayedLayerSystem.setLayerActive(5, true);
            samples === null || samples === void 0 ? void 0 : samples.forEach(sample => {
            ...
            if (this.dataset.isSequential) {
                if (this.lineLayerSystem.getValue(vector.__meta__.lineIndex)) {
                    ...
                    if (this.colorAttribute != null) {
                        ...
                        if (this.vectorColorScheme instanceof Mapping_1.ContinuousMapping) {
                            vector.__meta__.intrinsicColor = null;
                        }
                        else if (this.vectorColorScheme instanceof Mapping_2.DiscreteMapping) {
    ...
    isPointVisible(vector) {
        ...
        return (this.vectorSegmentLookup[i] == null || this.vectorSegmentLookup[i].__meta__.detailVisible)
            ...
            && (this.vectorSegmentLookup[i] == null || UtilityFunctions_1.valueInRange(this.vectorSegmentLookup[i].vectors.length, this.pathLengthRange))
...
exports.PointVisualization = PointVisualization;
