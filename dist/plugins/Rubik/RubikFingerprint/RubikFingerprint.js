...
class RubikFingerprint extends React.Component {
    constructor(props) {
        super(props);
        ...
        for (var side = 0; side < sides.length; side++) {
            for (var i1 = 0; i1 < 3; i1++) {
                for (var j = 0; j < 3; j++) {
                    ...
                    this.canvasContext.fillRect(1 + ox + ((offsetMap[sides[side] + "XOffset"] * 3 + j) * size), 1 + oy + ((offsetMap[sides[side] + "YOffset"] * 3 + i1) * size), w - 2, h - 2);
                }
            }
        }
    }
    ...
    componentDidUpdate(prevProps) {
        if (!UtilityFunctions_1.arraysEqual(prevProps.vectors, this.props.vectors) && this.props.vectors) {
            this.renderToContext();
        }
    }
    render() {
        return React.createElement("canvas", { className: "RubikFingerprintCanvas", ref: this.canvasRef, style: {
                width: this.props.width ? this.props.width : 72,
                height: this.props.height ? this.props.height : 96
            } });
    }
}
exports.RubikFingerprint = RubikFingerprint;
