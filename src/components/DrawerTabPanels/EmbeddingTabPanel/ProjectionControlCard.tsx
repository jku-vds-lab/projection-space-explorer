  ...
  ({
    ...
    const genlabel = (step) => {
      ...
      const percent = Math.min(
        (step / projectionParams.iterations) * 100,
        100
      ).toFixed(1);
      return (
        <div>
          <div>{`${Math.min(step, projectionParams.iterations)}/${
            projectionParams.iterations
          }`}</div>
          <div>{`${percent}%`}</div>
          {msg && <div>Server: {msg}</div>}
        </div>
          ...
          <CardHeader
            ...
            action={
              <IconButton
                ...
                onClick={(e) => {
                  console.timeEnd(
                    "time elapsed to project the file " + dataset_name
                  );
                  onClose();
                }}
              >
