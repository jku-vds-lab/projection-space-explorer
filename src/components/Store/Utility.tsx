import { IBaseProjection, createLinearRangeScaler, SchemeColor } from "../..";
...
export class UtilityActions {
  ...
  static generateImage(state: any, width: number, height: number, padding: number, options: any, ctx?: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D) {
    try {
        ...
        if (state.channelSize) {
            pointSizeScaler = createLinearRangeScaler(state.globalPointSize as any, state.dataset.columns[state.channelSize.key].range.min, state.dataset.columns[state.channelSize.key].range.max)
        }

        let isSelected = (value: number) => true
        ...
        state.projections.workspace.forEach((value, index) => {
            ...
            ctx.arc(sx(x), sy(y), (options?.pointSize ?? 4) * (state.channelSize ? pointSizeScaler(state.dataset.vectors[index][state.channelSize.key]) : 1), 0, 2 * Math.PI)
            ctx.fill()
            ctx.stroke();
