...
import {
  setPathLengthMaximum,
  setPathLengthRange,
} from "./components/Ducks/PathLengthRange";
...
} from "./components/Ducks/ProjectionDuck";
...
const mapDispatchToProps = (dispatch) => ({
  ...
  setAdvancedColoringSelection: (value) =>
    dispatch(setAdvancedColoringSelectionAction(value)),
  setActiveLine: (value) => dispatch(setActiveLine(value)),
  setProjectionColumns: (projectionColumns) =>
    dispatch(setProjectionColumns(projectionColumns)),
  setProjectionOpen: (projectionOpen) =>
    dispatch(setProjectionOpenAction(projectionOpen)),
  ...
  setChannelBrightness: (channelBrightness) =>
    dispatch(setChannelBrightnessSelection(channelBrightness)),
  saveProjection: (embedding: IProjection) =>
    dispatch(addProjectionAction(embedding)),
  updateWorkspace: (raw: IBaseProjection) =>
    dispatch(updateWorkspaceAction(raw)),
  setLineByOptions: (options) => dispatch(setLineByOptions(options)),
  setGlobalPointBrightness: (value) =>
    dispatch(setGlobalPointBrightness(value)),
  setGenericFingerprintAttributes: (value) =>
    dispatch(setGenericFingerprintAttributes(value)),
  setGroupVisualizationMode: (value) =>
    dispatch(setGroupVisualizationMode(value)),
...
export const Application = connector(
  class extends React.Component<Props, any> {
    ...
    componentDidMount() {
      if (
        (this.props.config?.preselect?.initOnMount ?? true) &&
        (this.props.config?.preselect?.url ?? false)
      ) {
        var preselect = this.props.config?.preselect?.url;

        var loader = preselect.endsWith(".csv")
          ? new CSVLoader()
          : new JSONLoader();

        const entry = DatasetEntriesAPI.getByPath(
          this.props.datasetEntries,
          preselect
        ) ?? {
    ...
    initializeEncodings(dataset) {
      ...
      var defaultSizeAttribute = CategoryOptionsAPI.getAttribute(
        dataset.categories,
        "size",
        "multiplicity",
        "sequential"
      ...
      var defaultColorAttribute = CategoryOptionsAPI.getAttribute(
        dataset.categories,
        "color",
        "algo",
        "categorical"
      ...
      var defaultBrightnessAttribute = CategoryOptionsAPI.getAttribute(
        dataset.categories,
        "transparency",
        "age",
        "sequential"
      ...
      if (defaultBrightnessAttribute) {
        ...
        this.threeRef.current.particles.transparencyCat(
          defaultBrightnessAttribute,
          [0.25, 1]
        );
      } else {
        ...
        >
            ...
            >
                ...
                title={
                  <React.Fragment>
                    ...
                    <Typography variant="body2">
                      Upload a new dataset or choose a predefined one.
                    </Typography>
                  </React.Fragment>
                }
              >
              ...
              <Tooltip
                ...
                title={
                  <React.Fragment>
                    ...
                    <Typography variant="body2">
                      Perform projection techniques like t-SNE, UMAP, or a
                      force-directly layout with your data.
                    </Typography>
                  </React.Fragment>
                }
              >
              ...
              <Tooltip
                ...
                title={
                  <React.Fragment>
                    <Typography variant="subtitle2">
                      Point and Line Channels
                    </Typography>
                    <Typography variant="body2">
                      Contains settings that let you map different channels like
                      brightness and color on point and line attributes.
                    </Typography>
                  </React.Fragment>
                }
              >
              ...
              <Tooltip
                ...
                title={
                  <React.Fragment>
                    ...
                    <Typography variant="body2">
                      Contains options for displaying and navigating groups in
                      the dataset.
                    </Typography>
                  </React.Fragment>
                }
              ...
              <Tooltip
                ...
                title={
                  <React.Fragment>
                    <Typography variant="subtitle2">
                      Hover Item and Selection Summary
                    </Typography>
                    <Typography variant="body2">
                      Contains information about the currently hovered item and
                      the currently selected summary.
                    </Typography>
                  </React.Fragment>
                }
              >
          ...
          >
            {this.props.overrideComponents?.appBar ? (
              ...
              <PseAppBar>
                <a href={"https://jku-vds-lab.at"} target={"_blank"}>
                  <VDSLogo style={{ height: 48, width: 48 }}></VDSLogo>
                </a>
                <Typography
                  variant="h6"
          ...
          {this.props.hoverStateOrientation ==
            HoverStateOrientation.SouthWest && (
            <div
              id="HoverItemDiv"
              style={{
                position: "absolute",
          ...
          {this.props.hoverStateOrientation ==
            HoverStateOrientation.NorthEast && (
            <div
              id="HoverItemDiv"
              style={{
                position: "absolute",
